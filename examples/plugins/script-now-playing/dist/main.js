...
async function restore(ctx) {
    ...
    await ctx.log.info(`session snapshot title=${track.title ?? 'none'} lines=${lyrics.lines.length} theme=${theme.source} pos=${player.positionMs}`);
    const stored = await ctx.storage.get('bookmark');
    let bookmark = null;
    try {
        bookmark = stored.value ? JSON.parse(stored.value) : null;
    }
    catch {
        await ctx.log.warn('bookmark JSON was unreadable');
    }
    const duration = track.durationMs ?? 0;
    const sameEntry = Boolean(bookmark?.queueEntryId) && bookmark?.queueEntryId === track.queueEntryId;
    ...
    activate(ctx) {
        const unsubscribers = [
            ...
            ctx.events.on('playback.positionCommitted', (payload) => {
                const positionMs = payload && typeof payload === 'object' && 'positionMs' in payload
                    ? Number(payload.positionMs)
                    : NaN;
                void Promise.all([ctx.track.get(), ctx.player.get()]).then(([track, player]) => ctx.storage.set('bookmark', JSON.stringify({
