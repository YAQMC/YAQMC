...
//!
//! Sensitive fields are also expected to be redacted at their source (URLs go through
//! [`crate::qqmusic::redaction::redact_url`], headers through
//! [`crate::qqmusic::redaction::redact_headers`], and so on). The redaction here is a
//! belt-and-suspenders layer, not the primary defense.
...
/// has started; unit tests should call `init_for_tests` instead.
...
pub fn sanitize_path(value: &str) -> String {
    ...
    if let Ok(userprofile) = std::env::var("USERPROFILE") {
        if !userprofile.is_empty() {
            result = result.replace(&userprofile, "<USER_HOME>");
        }
    }
    result
}
...
pub(crate) fn test_handle() -> LoggingHandle {
    LoggingHandle {
        session_id: Arc::from(generate_session_id()),
        log_dir: Arc::new(PathBuf::from("<test>")),
...
mod tests {
    ...
    #[test]
    fn sanitize_path_respects_home_env_variable() {
        let original = std::env::var_os("HOME");
        std::env::set_var("HOME", "/home/realuser");
        let cleaned = sanitize_path("/home/realuser/logs/yaqmc.log");
