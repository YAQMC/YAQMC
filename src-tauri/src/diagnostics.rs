...
use crate::{
    logging::{
        self, sanitize_path, scrub_high_risk_patterns, ErrorRecord, LogLevel, LoggingHandle,
        HIGH_RISK_KEYS, LOG_FILE_PREFIX,
    },
    platform::PlatformDiagnostics,
    qqmusic::ProviderStatus,
};
...
impl DiagnosticsSnapshot {
    ...
    pub fn assemble(
        ...
        Self {
            ...
            app,
            platform,
            provider: provider.map(|status| ProviderSection {
                id: status.provider_id,
                connection: status.connection,
                account_state: account_state.unwrap_or_else(|| "unknown".into()),
                membership_tier: provider_extras.as_ref().map(|value| value.0.clone()),
                membership_status: provider_extras.map(|value| value.1),
            }),
            playback,
            log_level,
            recent_errors,
...
pub fn export_bundle(
    ...
    let manifest_bytes = serde_json::to_vec_pretty(&manifest).map_err(BundleError::from_serde)?;
    write_entry(&mut writer, "manifest.json", &manifest_bytes, entry_options)?;
    ...
    redaction.files_scanned += 1;
    redaction.values_redacted += snapshot_stats.replacements;
    redaction
        .unresolved_patterns
        .extend(snapshot_stats.unresolved);
    write_entry(
        &mut writer,
        "diagnostics.json",
        &scrubbed_snapshot,
    ...
    redaction.files_scanned += 1;
    redaction.values_redacted += text_stats.replacements;
    redaction.unresolved_patterns.extend(text_stats.unresolved);
    write_entry(
        &mut writer,
        "diagnostics.txt",
        &scrubbed_text,
    ...
    if let Some(description) = options.description {
        ...
        redaction.files_scanned += 1;
        redaction.values_redacted += stats.replacements;
        redaction.unresolved_patterns.extend(stats.unresolved);
        write_entry(
            &mut writer,
    ...
    let mut total_bytes: u64 = manifest_bytes.len() as u64 + scrubbed_snapshot.len() as u64;
    if options.include_logs {
        for source in &log_files {
            let Some(name) = source.file_name().and_then(|value| value.to_str()) else {
                warnings.push(format!(
                    "skipped log with invalid file name: {}",
                    source.display()
                ));
                continue;
            };
    ...
    redaction.unresolved_patterns.sort();
    redaction.unresolved_patterns.dedup();
...
pub(crate) fn stub_platform_diagnostics() -> PlatformDiagnostics {
    use crate::{
        ...
        platform::{AudioDiagnostics, PlatformCapabilities},
...
#[cfg(test)]
pub(crate) fn stub_provider_status() -> ProviderStatus {
    use crate::qqmusic::{AudioQualityPreference, CatalogProviderCapabilities};
    ProviderStatus {
...
mod tests {
    ...
    fn stub_diagnostics(session: &str) -> DiagnosticsSnapshot {
        DiagnosticsSnapshot::assemble(
            ...
            Some(stub_provider_status()),
            Some("authenticated".into()),
            Some(("green-diamond".into(), "active".into())),
            PlaybackSection {
                state: "playing",
    ...
    fn sanitize_user_description_scrubs_secrets_and_paths() {
        ...
        let cleaned = sanitize_user_description(
