...
主应用沉浸歌词是完整体验；桌面歌词和歌词岛是轻量 Electron `BrowserWindow` 渲染器。两者消费同一个 Rust
...
因为完整锁定窗口不能接收点击，右上方叠加独立 42×42 解锁 `BrowserWindow`。内容窗口仍完全点击穿透，只有该
小控件能输入；其沙箱 preload 与窗口角色 IPC ACL 只开放 `lyrics_surface_unlock`，不能读取播放器、账号或偏好文档。设置页保留
单窗/全部解锁，托盘保留全局恢复。
...
Windows 支持两种表面及点击穿透、topmost、透明、多屏几何。Linux 共用状态和 Electron host IPC，但 X11/Wayland
