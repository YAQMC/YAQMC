...
100%，露出右侧空白。覆盖填满会铺满应用视觉视口并可能裁切边缘；完整显示会保留整张图片，比例不一致时
可能留边。留边是预期行为，不是显示故障。未覆盖区域使用回退色（应用外壳用外观背景色，歌词舞台用预设
回退色），而不是 WebView 的裸黑色。

外观中的图片适应仍控制应用外壳。歌词舞台使用当前[歌词预设](lyrics-presets.md)的 `background.fit`，并由
[歌词编排器](lyrics-composer.md)与沉浸歌词页共享同一场景。内置沉浸预设默认覆盖填满；已有的完整显示
偏好会作为用户覆盖迁移，不会被静默改写。滑条拇指始终可见，字号/行距不必悬停才找得到。
