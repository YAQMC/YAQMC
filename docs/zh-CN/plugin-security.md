...
第三方脚本**不会**在主渲染器中执行。每个脚本插件使用 YAQMC 自有 bootstrap 加上 `dist/main.js` 的独立 worker。
worker 没有 `document`、旧宿主全局对象、原始 IPC 或原始 `fetch`。特权操作必须经过绑定了运行时令牌的 `plugin_bridge`。插件自行声称的
`pluginId` 不能作为授权依据。应用 CSP 允许 `worker-src 'self' blob:`，以便 Chromium 创建该 blob worker；Worker
创建失败时插件标为 Failed。
