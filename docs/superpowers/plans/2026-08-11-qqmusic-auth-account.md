...
- Consumes: approved design source pins `L-1124/QQMusicApi@108617ffe80abefec6358717b9f4d3677550db10`, `wxuyu/QQMusicApi@44c3b26c8741521266c63002844564392a1fa38c`, and `RethinkQAQ/allmusic-qqmusicapi@a828f1f2d2dc8416bd1a549ee4c14efbb8ba4974`, plus the two narrowly scoped current corroborators `tlyanyu/multiPlatformMusicApi@0fd583b384f5d6477067ff3d29ccedd97fc3a317` for owned-playlist rename and `wangwalk/qqm@4a434ccf7468af29731a9792917cb6fc5a126bab` for account recent history.
...
- Produces: a factual `README.md` acknowledgement stating the five repositories were protocol references and no source was copied; account capability remains described as pending until the deterministic and live gates pass. It records the two corroborators with no GitHub-detected license as behavior-only research references with no source reuse.
...
$sources = @(
  ...
  @{ Repo = 'tlyanyu/multiPlatformMusicApi'; Sha = '0fd583b384f5d6477067ff3d29ccedd97fc3a317' },
  @{ Repo = 'wangwalk/qqm'; Sha = '4a434ccf7468af29731a9792917cb6fc5a126bab' }
)
...
Create `scripts/check-qqmusic-reference-evidence.ps1` before running those commands. The script owns four PowerShell classes: `ReferenceTarget` (`Repository`, `Commit`, `Path`), `SymbolSelector` (`Language`, `Path`, `AnchorKind`, `Anchor`, `EvidenceClass`, `ExpectedSymbols`), `DeclarationEdge` (`FromPath`, `FromAnchor`, `EdgeKind`, `ToPath`, `ToAnchor`), and `OperationEvidenceSpec` (`Operation`, `Selectors`, `RequiredResponseKeys`, `RequiredDeclarationEdges`, `SecretHeaderPolicy`, `PaginationPolicy`, `MinimumCorroboratingSources`). Its immutable `ReferenceTarget[]` contains the five exact repositories, full 40-character commits, every declaration path listed below, and only the statically followed common decoder/helper paths named in the next paragraph; callers cannot inject or replace targets. It downloads each pinned body into memory, hashes the bytes, passes the text directly to a language-aware token extractor, then releases the body. It must never add an `operations` field to a source record and must never accept an operation name found in source text as evidence.

Resolve target ownership by an immutable exact-path map, never by a caller-supplied repository or basename: `qqmusic_api/**` belongs to the pinned `L-1124/QQMusicApi`; `src/services/**` belongs to the pinned `wxuyu/QQMusicApi`; `src/main/java/qqmusicapi/QQMusicLoginHelper.java` belongs to the pinned `RethinkQAQ/allmusic-qqmusicapi`; `platforms/qqmusic/module/playlist_update.js`, `platforms/qqmusic/module/user_playlist_created.js`, `platforms/base/BasePlatform.js`, `platforms/qqmusic/QQMusicPlatform.js`, and `platforms/qqmusic/util/request.js` belong to the pinned `tlyanyu/multiPlatformMusicApi`; and `src/api/user.ts` plus `src/api/client.ts` belong to the pinned `wangwalk/qqm`. Reject duplicate exact paths across targets at script startup. The manifest may refer only to these immutable path identities.

The only non-row helper targets are `qqmusic_api/models/login.py` for `QRCodeLoginEvents`, `qqmusic_api/core/request.py` for the shared CGI result decoder, `qqmusic_api/core/versioning.py` for the statically followed authenticated `comm` envelope, and `src/util/request.ts` for the imported `cookie -> headers.Cookie` request-wrapper edge. They may contribute only their named decoder/header symbols after the row declaration proves the corresponding static enum/import/call edge; they do not increase a row's corroborating-source count. Python-only CGI rows prove that the pinned client carries the conceptual credential through the CGI payload/`comm` path and emits no secret-bearing request header, so their header cell is the literal `none`; this does not mean that the request is unauthenticated or that its body is non-secret.
...
- The ECMAScript scanner handles TypeScript and JavaScript, skips comments/template contents, balances braces/parentheses across lines, and reads both quoted keys and unquoted identifier keys from multiline object literals. It recognizes named TypeScript exports, class methods, `const` function declarations, and CommonJS `module.exports` functions; it associates `fetch`/client request configuration, delegated request calls, and `headers` objects with the enclosing anchored declaration. Static imports/`require` calls may be followed only into another immutable target in the same pinned repository. For the one pinned dynamic module loader, it may evaluate only the literal transformation `playlist_update.js -> playlist/update`, then prove `loadModules` stores that exact module export, `callModule` retrieves it and injects `this.createRequestFunction()`, the QQ subclass returns a delegate to the statically required `request.createRequest`, and that symbol resolves to the pinned `const createRequest`; no other dynamic path, runtime value, inheritance target, or module name is accepted.
...
- Each scanner emits only symbol names: HTTPS origin/path literals (query values removed), paired `module/method` literals from one request object, request-key names, response-key names read from an anchored response object, pagination-key names, request-header names, response-header names, validated result literals, declaration name, declaration-edge identities, content SHA-256, and provenance. It never emits a literal value belonging to a request key/header, an excerpt, a body, a cookie, or QR bytes.
...
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- | ------------------------------ | -------------------------------- | ---------------------------------------------------------------------------------------------------------------------- | --------------- |
| QR create                  | `qqmusic_api/modules/login.py#py:def:_get_qq_qr`; `src/services/apis/user/getQQLoginQr.ts#ts:export:getQQLoginQr`                                                                                                                                                                                                                                                                                                                                                                                                                                            | `url:https://ssl.ptlogin2.qq.com/ptqrshow`                                                                                                                                                              | `keys:appid,daid,pt_3rd_aid,u1`                                        | `none`                         | `none`                           | `results:200`                                                                                                          | 2               |
| QR status                  | `qqmusic_api/modules/login.py#py:def:_check_qq_qr`; `src/services/apis/user/checkQQLoginQr.ts#ts:export:checkQQLoginQr`                                                                                                                                                                                                                                                                                                                                                                                                                                      | `url:https://ssl.ptlogin2.qq.com/ptqrlogin`                                                                                                                                                             | `keys:ptqrtoken,qrsig,action,u1`                                       | `headers:Cookie`               | `none`                           | `results:65,66,67,68,QRCodeLoginEvents.SCAN,QRCodeLoginEvents.CONF,QRCodeLoginEvents.TIMEOUT,QRCodeLoginEvents.REFUSE` | 2               |
...
| Favorites write            | `qqmusic_api/modules/songlist.py#py:def:like_song`; `qqmusic_api/modules/songlist.py#py:def:unlike_song`                                                                                                                                                                                                                                                                                                                                                                                                                                                     | `mm:music.musicasset.PlaylistDetailWrite/AddSonglist` or `mm:music.musicasset.PlaylistDetailWrite/DelSonglist`                                                                                          | `keys:dirId,songId,songType`                                           | `none`                         | `none`                           | `results:0`                                                                                                            | 1               |
| playlist summaries         | `qqmusic_api/modules/user.py#py:def:get_created_songlist`; `src/services/apis/user/getUserPlaylists.ts#ts:export:getUserPlaylists`; `src/services/apis/user/getUserCollections.ts#ts:export:getUserCollectedSongLists`                                                                                                                                                                                                                                                                                                                                       | `mm:music.musicasset.PlaylistBaseRead/GetPlaylistByUin` or `url:https://c6.y.qq.com/rsc/fcgi-bin/fcg_get_profile_homepage.fcg` or `url:https://c.y.qq.com/fav/fcgi-bin/fcg_get_profile_order_asset.fcg` | `keys:uin,sin,ein`                                                     | `headers:Cookie`               | `keys:offset,limit,page,sin,ein` | `results:0`                                                                                                            | 2               |
| playlist detail            | `qqmusic_api/modules/songlist.py#py:def:get_detail`; `src/services/apis/songLists/songListDetail.ts#ts:default-export`                                                                                                                                                                                                                                                                                                                                                                                                                                       | `mm:music.srfDissInfo.DissInfo/CgiGetDiss`                                                                                                                                                              | `keys:disstid,song_begin,song_num`                                     | `none`                         | `keys:song_begin,song_num`       | `results:0`                                                                                                            | 2               |
| playlist create            | `qqmusic_api/modules/songlist.py#py:def:create`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `mm:music.musicasset.PlaylistBaseWrite/AddPlaylist`                                                                                                                                                     | `keys:dirName`                                                         | `none`                         | `none`                           | `results:0`                                                                                                            | 1               |
| playlist rename            | `platforms/qqmusic/module/playlist_update.js#js:module-exports`; `platforms/qqmusic/module/user_playlist_created.js#js:module-exports`; `platforms/base/BasePlatform.js#js:class:BasePlatform`; `platforms/base/BasePlatform.js#js:method:BasePlatform.loadModules`; `platforms/base/BasePlatform.js#js:method:BasePlatform.callModule`; `platforms/qqmusic/QQMusicPlatform.js#js:class:QQMusicPlatform`; `platforms/qqmusic/QQMusicPlatform.js#js:method:QQMusicPlatform.createRequestFunction`; `platforms/qqmusic/util/request.js#js:const:createRequest` | `mm:music.musicasset.PlaylistBaseRead/GetPlaylistByUin` and `mm:music.musicasset.PlaylistBaseWrite/EditPlaylist`                                                                                        | `keys:uin,dirId,mask,dirNewName,dirNewDesc,dirNewPicUrl,dirNewtaglist` | `headers:Cookie`               | `none`                           | `results:0`                                                                                                            | 1               |
| playlist add               | `qqmusic_api/modules/songlist.py#py:def:add_songs`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | `mm:music.musicasset.PlaylistDetailWrite/AddSonglist`                                                                                                                                                   | `keys:dirId,songId,songType`                                           | `none`                         | `none`                           | `results:0`                                                                                                            | 1               |
| playlist remove            | `qqmusic_api/modules/songlist.py#py:def:del_songs`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | `mm:music.musicasset.PlaylistDetailWrite/DelSonglist`                                                                                                                                                   | `keys:dirId,songId,songType`                                           | `none`                         | `none`                           | `results:0`                                                                                                            | 1               |
| playlist delete            | `qqmusic_api/modules/songlist.py#py:def:delete`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `mm:music.musicasset.PlaylistBaseWrite/DelPlaylist`                                                                                                                                                     | `keys:dirId`                                                           | `none`                         | `none`                           | `results:0`                                                                                                            | 1               |
| recent history             | `src/api/user.ts#ts:export:getRecentTracks`; `src/api/client.ts#ts:method:ApiClient.request`                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | `mm:music.musichallSong.RecentPlayList/GetRecentPlayList`                                                                                                                                               | `keys:uin,begin,num`                                                   | `headers:Cookie`               | `keys:begin,num`                 | `results:0`                                                                                                            | 1               |
| entitlement                | `qqmusic_api/modules/user.py#py:def:get_vip_info`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | `mm:VipLogin.VipLoginInter/vip_login_base`                                                                                                                                                              | `keys:uin`                                                             | `none`                         | `none`                           | `results:0`                                                                                                            | 1               |
| playback vkey              | `qqmusic_api/modules/song.py#py:def:get_song_urls`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | `mm:music.vkey.GetVkey/UrlGetVkey` or `mm:music.vkey.GetEVkey/CgiGetEVkey`                                                                                                                              | `keys:filename,guid,songmid,uin`                                       | `none`                         | `none`                           | `results:0`                                                                                                            | 1               |

The playlist-rename manifest requires response keys `dirId,dirName,picUrl,desc,tagNameList` from the object mapped out of `GetPlaylistByUin` and hard-codes these seven `RequiredDeclarationEdges` in order: `QQMusicPlatform --inherits--> BasePlatform`; `BasePlatform.loadModules --loads-route:playlist/update--> playlist_update.js#module.exports`; `BasePlatform.callModule --retrieves-route:playlist/update--> playlist_update.js#module.exports`; `BasePlatform.callModule --virtual-call-on:QQMusicPlatform--> QQMusicPlatform.createRequestFunction`; `BasePlatform.callModule --injects-return-of:QQMusicPlatform.createRequestFunction-as-parameter:request--> playlist_update.js#module.exports`; `playlist_update.js#module.exports --static-require-call-forwarding:query,request--> user_playlist_created.js#module.exports`; and `QQMusicPlatform.createRequestFunction --static-require-delegate--> request.js#createRequest`. The scanner derives `playlist/update` only from the pinned filename transformation, requires both module functions' `request(...)` calls to target the same forwarded injected parameter, and associates the required response keys only with `response.body.v_playlist` mapping. A same-named declaration, unconnected transport record, or unrelated object key cannot satisfy any edge/response key.

The validator resolves every selector against extracted tokens, unions only the resolved records for that operation, and then requires: every declared normalized endpoint or paired module/method (not merely one when the manifest uses `and`); every required request key; every `RequiredResponseKey`; every required secret-bearing **request** header name (or proven `none`); pagination keys or proven `none`; at least one valid result literal; every ordered `RequiredDeclarationEdge`; and the minimum number of distinct `repository@commit:path` corroborators. Playlist rename requires the exact ordered metadata-read/dynamic-load/injected-delegate/static-require chain from `GetPlaylistByUin` through `playlist_update.js`, `BasePlatform.callModule`, `QQMusicPlatform.createRequestFunction`, and `request.js#createRequest`; endpoint, key, response-key, header, and result tokens from disconnected declarations cannot be unioned into passing evidence. A selector whose anchor, edge, or expected symbol is absent is a hard failure; malformed selectors, duplicate operation names, unknown evidence classes, empty expected-symbol arrays, cyclic/unreachable declaration chains, and source-count shortfalls are hard failures. No path-to-operation tag or caller-supplied `operations` property participates in resolution.
...
`-SelfTest` uses only synthetic strings and deep-cloned manifests. It proves a multiline TypeScript request with unquoted `module`, `method`, `params`, `page`, and `pageSize` keys is extracted; a multiline CommonJS fixture proves the exact filename-to-route load, module-map lookup, injected request parameter, forwarded metadata-read delegate, subclass delegate, static `require`, and `const createRequest` chain; a response-side `Set-Cookie` never satisfies `headers:Cookie`; a fabricated `operations = @('playlist rename')` tag never creates evidence; deleting each evidence class in turn fails; reducing corroboration below the declared minimum fails; and every noisy token in the deny-list plus an arbitrary bare identifier fails result validation. It also mutates a selector to an unknown path/anchor and proves that the fabricated operation cannot pass. The two anchor regressions explicitly replace the real playlist selector with `qqmusic_api/modules/songlist.py#py:def:rename` and the real recent selector with `qqmusic_api/modules/user.py#py:def:get_recent_song`; each must hard-fail because those declarations do not exist at the pinned commit. `disconnected-rename-delegate` retains all rename declarations and their endpoint/key/response-key/header/result tokens but removes the `QQMusicPlatform.createRequestFunction -> request.createRequest` edge; it must fail, proving that record union alone cannot self-certify the operation. `missing-rename-preservation-field` removes only `tagNameList` from the anchored `v_playlist` mapper and must fail while every other rename token remains. Each `-SelfTestMutation` invocation must print `EXPECTED_REJECTION:<name>` and exit zero only after its malformed candidate is rejected; accidental acceptance exits nonzero.

Expected: both commands PASS. Nontruncating JSON contains each full 40-character commit and 64-character content SHA-256 plus only resolved endpoint paths, paired module/method literals, request-header names, response-header names, request-key names, response-key names, pagination-key names, validated result literals, declaration-edge identities, and exact source corroboration. It contains no operation tags, header/request values, source excerpt, reference source body, response body, cookie value, or QR artifact and writes nothing to the repository or `output/`.
...
Each row records current observable protocol behavior, not a supported public SDK. GPL/LGPL sources and sources for which GitHub detects no license were used only to locate behaviors for independent observation; YAQMC's Rust implementation is original, and no source was copied or vendored.
...
QQ Music interoperability research consulted `L-1124/QQMusicApi`, `wxuyu/QQMusicApi`, `RethinkQAQ/allmusic-qqmusicapi`, `tlyanyu/multiPlatformMusicApi`, and `wangwalk/qqm` at the commits and license-detection results recorded in `docs/qqmusic-provider.md`. They were used as protocol-behavior references; YAQMC does not copy or vendor their implementations, and the projects do not endorse YAQMC.
```
...
- Generate: `src-tauri/permissions/autogenerated/*.toml` for the 16 exact account commands
...
The repository already exposes `commands::qqmusic_sign_out`, so change that command now to accept an injected
`tauri::WebviewWindow` and call `require_main_window(&window)?` before reading provider state. This makes the Rust
guard active for the only account command that exists before Tasks 6–12; every later account command follows the
same pattern when introduced.
...
Run `cargo check` once after adding the app manifest. Tauri must generate exactly 16
`src-tauri/permissions/autogenerated/*.toml` command permission pairs and refresh the four tracked schema snapshots
listed in this task. Commit those deterministic outputs so a clean checkout does not become dirty after the next
build. Reject any generated command outside `ACCOUNT_COMMANDS`.
...
$generated = @(Get-ChildItem 'src-tauri/permissions/autogenerated' -Filter '*.toml')
if ($generated.Count -ne 16) { throw "Expected 16 generated account permissions, got $($generated.Count)" }
$schemas = @(
...
foreach ($schema in $schemas) {
  if ((Get-Content -Raw $schema) -notmatch 'qqmusic-account') {
    throw "Generated schema is missing qqmusic-account: $schema"
  }
}
```
...
```powershell
git add src-tauri/build.rs src-tauri/permissions/qqmusic-account.toml src-tauri/permissions/autogenerated src-tauri/capabilities/main-window.json src-tauri/gen/schemas/acl-manifests.json src-tauri/gen/schemas/capabilities.json src-tauri/gen/schemas/desktop-schema.json src-tauri/gen/schemas/windows-schema.json src-tauri/src/command_guard.rs src-tauri/src/commands.rs src-tauri/src/lib.rs
...
- Produces the exact cross-language `ProviderErrorCode` set: retain `offline`, `timeout`, `authentication-expired`, `entitlement-unavailable`, `rate-limited`, `schema-changed`, `song-unavailable`, `malformed-response`, and `provider-failure`; replace the unused `unauthorized` spelling with `authorization-rejected`; add `cancelled`, `not-found`, `invalid-request`, `unsupported-operation`, `mutation-in-progress`, and `storage-failure`. `unsupported-operation` is non-retryable and represents an intentionally unavailable normalized capability, including a tagged-playlist rename for which no lossless current tag-ID round trip is verified. `cancelled` is non-retryable and is returned whenever an account operation's captured auth epoch is no longer current. Mutation uncertainty remains `MutationStatus::OutcomeUnknown`, not a generic provider error.
...
- Produces internal `RawOwnedPlaylistEditFields { provider_dir_id: Option<u64>, original_title: Option<String>, description: Option<String>, picture_url: Option<String>, tag_names: Option<Vec<String>> }`, decoded directly from provider keys `dirId`, `dirName`, `desc`, `picUrl`, and `tagNameList`; it derives `Deserialize` but neither `Debug` nor `Serialize`. `parse_owned_playlist_edit_snapshot(&raw) -> Result<ProviderOwnedPlaylistEditSnapshot, PlaylistEditSafetyError>` returns `ProviderOwnedPlaylistEditSnapshot { provider_dir_id: u64, original_title: String, description: String, picture_url: String, tag_names: Vec<String> }` only when every field is present and `tagNameList` is empty. `PlaylistEditSafetyError` derives `Clone, Copy, Debug, PartialEq, Eq`; a present empty string/list is valid, an absent field is `SchemaChanged`, and nonempty tags are `TaggedRenameUnverified`. Task 12 maps those errors to `ProviderErrorCode::SchemaChanged` and `ProviderErrorCode::UnsupportedOperation`, respectively, and never guesses a tag name-to-ID mapping. The snapshot derives neither `Debug` nor `Serialize`, is never logged/cached/exposed to React, and is discarded after capability derivation or one Task 12 mutation.
...
fn owned_playlist_without_lossless_edit_metadata_disables_only_rename() {
    for (raw, expected) in [
        (owned_playlist_fixture_missing_tag_field(), PlaylistEditSafetyError::SchemaChanged),
        (owned_playlist_fixture_with_tags(), PlaylistEditSafetyError::TaggedRenameUnverified),
    ] {
        ...
        assert!(matches!(parse_owned_playlist_edit_snapshot(&raw), Err(error) if error == expected));
    }
}
...
PlaylistCapabilities {
    ...
    can_rename: ownership == PlaylistOwnership::Owned
        && upstream.can_rename
        && parse_owned_playlist_edit_snapshot(raw).is_ok(),
    can_delete: ownership == PlaylistOwnership::Owned && upstream.can_delete,
    can_reorder: false,
}
```

Keep `can_reorder` false until independently verified; do not infer ownership from display name. A summary that lacks the full edit snapshot reports `canRename: false`; loading a fresh detail may upgrade it to true only when every raw edit field is present. Empty description/picture/tag values count as present and remain distinguishable from missing schema fields.
...
- Reconciliation queries: create -> playlist summaries diff; rename -> playlist detail title plus exact description/picture/tag equality against the captured `ProviderOwnedPlaylistEditSnapshot`; add/remove -> paged playlist track membership; delete -> playlist summaries absence.
...
- Rename keeps the frontend DTO title-only but never sends a title-only native wire request. Under the playlist mutex it fetches a fresh owned-playlist detail, parses Task 9's complete provider-native edit snapshot, and builds internal `EditPlaylistPayload { dir_id: u64, mask: u8, dir_new_name: String, dir_new_desc: String, dir_new_pic_url: String, dir_new_tag_list: String }`. Its `Serialize` field names are exactly `dirId`, `mask`, `dirNewName`, `dirNewDesc`, `dirNewPicUrl`, and the upstream spelling `dirNewtaglist`; the last value is the empty string only after the safety parser proved the playlist's present `tagNameList` is empty. Missing fields return `SchemaChanged`; any tagged playlist returns `UnsupportedOperation` before transport and forces `canRename: false`. The implementation never substitutes normalized artwork, fallback artwork, empty defaults for absent values, a tag-name lookup table, or cached stale metadata.
...
#[tokio::test]
async fn rename_preserves_description_picture_and_empty_tags_on_the_wire_and_readback() {
    let fixture = playlist_mutation_fixture_with_untagged_edit_snapshot(
        "Original description",
        "https://qpic.y.qq.com/synthetic-owned.png",
    );
    let result = fixture.service().rename_playlist(rename_request_with_title("Renamed")).await.expect("rename");
    let payload = fixture.transport().only_rename_payload();
    assert_eq!(payload.mask, 15);
    assert_eq!(payload.dir_new_name, "Renamed");
    assert_eq!(payload.dir_new_desc, "Original description");
    assert_eq!(payload.dir_new_pic_url, "https://qpic.y.qq.com/synthetic-owned.png");
    assert_eq!(payload.dir_new_tag_list, "");
    assert!(matches!(result.status, MutationStatus::Applied | MutationStatus::Reconciled));
    let readback = fixture.readback_edit_snapshot();
    assert_eq!(readback.description, "Original description");
    assert_eq!(readback.picture_url, "https://qpic.y.qq.com/synthetic-owned.png");
    assert!(readback.tag_names.is_empty());
}

#[tokio::test]
async fn incomplete_edit_metadata_disables_rename_and_sends_no_write() {
    let missing = playlist_mutation_fixture_missing_edit_tags();
    let result = missing.service().rename_playlist(rename_request_with_title("Do not send")).await;
    assert!(matches!(result, Err(QQMusicError::SchemaChanged)));
    assert_eq!(missing.transport().rename_calls(), 0);
    assert!(!missing.latest_detail().summary.capabilities.can_rename);

    let tagged = playlist_mutation_fixture_with_tags();
    let result = tagged.service().rename_playlist(rename_request_with_title("Still do not send")).await;
    assert!(matches!(result, Err(QQMusicError::UnsupportedOperation)));
    assert_eq!(tagged.transport().rename_calls(), 0);
    assert!(!tagged.latest_detail().summary.capabilities.can_rename);
}
```

- [ ] **Step 2: Write failing frontend capability/pending/rollback tests**

Assert collected playlists render no Rename/Delete/Add/Remove actions. Assert an owned tagged or edit-schema-incomplete playlist hides Rename while retaining independently valid owner actions such as Delete. Assert enabled owned controls call exactly one provider method, remain disabled under their per-entity key, rollback definite rejection, and refresh after unresolved unknown.
...
For rename specifically, fetch the fresh provider-native detail only after acquiring `playlist:<id>`, require owned status plus a complete untagged `ProviderOwnedPlaylistEditSnapshot`, and copy its description and raw picture URL value-for-value into the `EditPlaylist` payload while changing only `dirNewName`; set `dirNewtaglist` to empty only because the parser proved the provider returned a present empty tag list. Use `mask: 15` only with all fields populated from that snapshot. After send, re-read detail and accept applied/reconciled only when the title matches, description/picture remain identical, and tags remain present-empty; a mismatched or unreadable readback is `OutcomeUnknown`, leaves cached data stale, and emits no automatic second write. Tagged playlists keep `canRename: false` and never enter transport. Add/remove/delete do not reuse this metadata path.
...
assert!(snapshot.tag_names.is_empty());
let dir_new_tag_list = String::new();
```
...
Expected: create/rename/add/remove/delete success, rejection, unknown reconciliation, shared same-playlist serialization, logout/login-swap cancellation at write/reconciliation/final-commit barriers, duplicate suppression, collected-playlist denial, exact safe-rename preservation of description/picture/empty-tag state, no rename write when metadata is incomplete or tags are nonempty, and temporary lifecycle safety PASS.
...
At every step assert `ownership == owned`, the ID equals the newly created ID, and the title begins `YAQMC Integration Test (`. Never rename/delete any ID present in the pre-write snapshot. After track removal, fetch a fresh provider-native detail under the playlist lock and require the exact safe-rename preconditions: every edit field is present and `tagNameList` is explicitly empty. Only then may the UI advertise `canRename`, send `mask: 15`, preserve the fresh description/picture/empty-tag state value-for-value, and accept the rename after a readback proves that only the title changed. Missing metadata, a nonempty tag list, absent `canRename`, preservation mismatch, or unreadable readback records `playlist-write` as `fail` with `endpoint-changed` or `capability-not-advertised`; it never sends a lossy rename.

Record `playlist-write` as `pass` only after create, add/read, remove/read, safe rename/readback, and every identity lock succeed. Whether an intermediate write passes, fails, or remains unknown, once the temporary ID is uniquely known the next safe write is the one cleanup exception: attempt exactly one delete for that same ID, then perform bounded read-only reconciliation until absence is confirmed or the cleanup bound expires. This cleanup attempt must still run after a rename/precondition failure; do not hide a cleanup failure behind the earlier operation result. On unresolved create outcome, reconcile summaries without a second create; delete only if exactly one new matching ID is established. Record `temporary-playlist-cleanup` independently as `pass` only after confirmed absence, otherwise `fail` plus `cleanup-failed`. The recorder deliberately has no field that could accept the playlist ID or title; the human-facing report may identify only the temporary ID needed for manual cleanup and must expose no other playlist data.
