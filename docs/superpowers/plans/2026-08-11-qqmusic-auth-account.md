...
- The ECMAScript scanner handles TypeScript and JavaScript, skips comments/template contents, balances braces/parentheses across lines, and reads both quoted keys and unquoted identifier keys from multiline object literals. It recognizes named TypeScript exports, class methods, `const` function declarations, and CommonJS `module.exports` functions; it associates `fetch`/client request configuration, delegated request calls, and `headers` objects with the enclosing anchored declaration. Static imports/`require` calls may be followed only into another immutable target in the same pinned repository.
...
| playlist rename            | `platforms/qqmusic/module/playlist_update.js#js:module-exports`; `platforms/qqmusic/util/request.js#js:const:createRequest`                                                                                            | `mm:music.musicasset.PlaylistBaseWrite/EditPlaylist`                                                                           | `keys:dirId,mask,dirNewName,dirNewDesc,dirNewPicUrl,dirNewtaglist` | `headers:Cookie`               | `none`                     | `results:0`                                                                                                            | 1               |
