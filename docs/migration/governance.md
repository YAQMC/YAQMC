...
- YAQMC records an **optional** `qqmusic-api` git pin at `ffcc86c` behind feature `qmapi`. Default Core stays `intree` and must not resolve `qqmusic-api`. That optional pin is not a default backend change and not P14-C.
