...
- At pin `56db511` (unchanged at `ffcc86c`), `qqmusic-api` declares `rust-version = "1.88.0"` and upstream CI tests on that toolchain. That is metadata, not provenance clearance.
...
- YAQMC pins `qqmusic-api` unconditionally at `ffcc86c` since the P14-C
  cutover; the backend feature split and `qqmusic-qmapi` opt-in were removed.
