...
| qm-api-rs                     | The repository is private. Current YAQMC pin is `476b37e3135560dff132e9ba8996e068af706458` (tolerant clear-vkey parsing on top of the `ffcc86c` independent-implementation record). Crate `qqmusic-api` (lib `qqmusic_api`), version `0.1.0`, GPL-3.0-or-later, unconditional since the P14-C cutover. QMC provenance and the L-1124 record are complete. CI needs authenticated dependency access. SHA1 `zzc_sign` differs from the in-tree MD5 `zzb` signer, so A/B signing remains in-tree. |
...
- At pin `56db511` (unchanged through `ffcc86c`/`476b37e`), `qqmusic-api` declares `rust-version = "1.88.0"` and upstream CI tests on that toolchain. That is metadata, not provenance clearance.
...
- YAQMC has linked `qqmusic-api` unconditionally since the P14-C cutover and
  currently pins it at `476b37e`; the backend feature split and
  `qqmusic-qmapi` opt-in were removed.
