...
The Rust Provider Components have separate reproducible targets. Use `npm run plugin:pack:provider-example` for the
read-only catalog and `npm run plugin:verify:provider-platform-example` for the complete capability fixture.
...
| [Complete platform](../examples/plugins/packages/dev.yaqmc.example.platform-1.0.0.yaqmc-plugin)   | Adds a deterministic platform with playable local audio | API v3 catalog including Discover, playback, recommendations, lyrics, private storage, and a synthetic isolated account |
...
Provider Component source is in `examples/plugins/provider-catalog-rust` and
`examples/plugins/provider-platform-rust`. Install the same package on supported Windows or Linux architectures;
neither contains a native binary. The complete example uses `accounts.example.com` only and its OAuth callback is
completed by automated tests—it is not a real account service. Its media URL/path never enters the renderer: the
Component returns a Host cache recipe and Core consumes the resulting opaque source. Hostile probes live under test
fixtures and must not be enabled as user plugins.

## Not included

Marketplace, remote updates, native modules, HTML entrypoints, raw `fetch`, real account integrations, and
