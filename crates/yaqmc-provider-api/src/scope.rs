...
use crate::{
    ...
    Page, Playlist, PlaylistMutationResult, ProviderCommandError, ProviderResult, ProviderStatus,
    ProviderTrackReference, RecommendationBatch, RemotePlayHistoryItem, SearchResult, ShareTarget,
    Song, DEFAULT_PROFILE_ID,
};
...
fn validate_provider_reference(
    ...
    validate_profile_id(&reference.profile_id)
}

fn validate_profile_id(profile_id: &str) -> ProviderResult<()> {
...
impl ProviderScopedOutput for Song {
    fn validate_scope(mut self, provider_id: &str) -> ProviderResult<Self> {
        if let Some(reference) = &self.provider {
            validate_provider_reference(reference, provider_id)?;
        } else {
...
impl ProviderScopedOutput for ShareTarget {
    fn validate_scope(self, provider_id: &str) -> ProviderResult<Self> {
        validate_provider_id_and_profile(&self.provider_id, &self.profile_id, provider_id)?;
        Ok(self)
    }
}
...
impl<T: ProviderScopedOutput> ProviderScopedOutput for Page<T> {
    fn validate_scope(mut self, provider_id: &str) -> ProviderResult<Self> {
        self.items = self
            .items
            .into_iter()
            .map(|item| item.validate_scope(provider_id))
...
impl ProviderScopedOutput for AccountPlaylistDetail {
    fn validate_scope(mut self, provider_id: &str) -> ProviderResult<Self> {
        self.summary = self.summary.validate_scope(provider_id)?;
        self.tracks = self.tracks.validate_scope(provider_id)?;
        Ok(self)
    }
}
...
impl ProviderScopedOutput for Album {
    fn validate_scope(mut self, provider_id: &str) -> ProviderResult<Self> {
        self.tracks = self
            .tracks
            .into_iter()
            .map(|song| song.validate_scope(provider_id))
            .collect::<ProviderResult<Vec<_>>>()?;
        Ok(self)
    }
}
...
impl ProviderScopedOutput for FeaturedRelease {
    fn validate_scope(mut self, provider_id: &str) -> ProviderResult<Self> {
        self.album = self.album.validate_scope(provider_id)?;
        Ok(self)
    }
}
...
impl ProviderScopedOutput for MediaCollection {
    fn validate_scope(self, provider_id: &str) -> ProviderResult<Self> {
        match self {
            Self::Album(album) => Ok(Self::Album(album.validate_scope(provider_id)?)),
            Self::Playlist(playlist) => Ok(Self::Playlist(playlist.validate_scope(provider_id)?)),
        }
    }
}
...
impl ProviderScopedOutput for HomeFeed {
    fn validate_scope(mut self, provider_id: &str) -> ProviderResult<Self> {
        self.featured = self.featured.validate_scope(provider_id)?;
        ...
        self.made_for_you = self
            .made_for_you
            .into_iter()
            .map(|playlist| playlist.validate_scope(provider_id))
            .collect::<ProviderResult<Vec<_>>>()?;
        self.new_releases = self
            .new_releases
            .into_iter()
            .map(|album| album.validate_scope(provider_id))
            .collect::<ProviderResult<Vec<_>>>()?;
...
impl ProviderScopedOutput for RemotePlayHistoryItem {
    fn validate_scope(mut self, provider_id: &str) -> ProviderResult<Self> {
        self.song = self.song.validate_scope(provider_id)?;
        Ok(self)
    }
}
